...
var PdfHighlighter = function (_PureComponent) {
  ...
  function PdfHighlighter() {
    ...
    return _ret = (_temp = (_this = _possibleConstructorReturn(this, _PureComponent.call.apply(_PureComponent, [this].concat(args))), _this), _this.state = {
      ...
      var getTopValue = function getTopValue(styleString) {
        var result = /top: .*?px;/.exec(styleString);
        if (!result || result.length === 0) return null;

        var value = result[0].replace("top: ", "").replace("px;", "");
      ...
      for (var i = 0; i < contentCollection.length; i++) {
        var lineBreak = "";
        if (i > 0) {
          var previousTop = getTopValue(contentCollection.item(i - 1).getAttribute("style"));
          var currentTop = getTopValue(contentCollection.item(i).getAttribute("style"));
          if (previousTop && currentTop && previousTop !== currentTop) lineBreak += "\r\n";
        }
        text += lineBreak + (i === 0 || lineBreak ? "" : " ") + contentCollection.item(i).textContent;
      }
  ...
  PdfHighlighter.prototype.findOrCreateHighlightLayer = function findOrCreateHighlightLayer(page) {
    var pageView = this.viewer.getPageView(page - 1);
